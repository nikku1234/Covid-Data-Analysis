...
# Next, define a baseURL from which we download the case data of the Coronavirus by country and region. 
# The data is located on a server of the Johns Hopkins Center for Systems Science and Engineering (JHU/CSSE). 
# The function loadData handles the download and some basic transformations, as explained below.
...
# read_csv downloads the file from the web and converts it into a Pandas dataframe. 
# Geo-coordinates are omitted by drop; melt converts the dataframe from wide to long format. 
# Then, astype ensures the types are correct.Finally, the method fillna inserts the text <all> if no region is available or 0 for the column containing the cumulated case number.
# The last transformation is a conversion into date type for the corresponding column.
# By calling the function loadData multiple times we load a cleaned dataframe of the Confirmed Cases, 
# Recovered Cases and Deaths. The function merge joins these 3 data sets to get one data set including all the data:
